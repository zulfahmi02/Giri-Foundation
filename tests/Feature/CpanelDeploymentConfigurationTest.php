...
test('cpanel deployment files exist and reference the deploy script', function () {
    ...
    expect($deploymentConfig)->toContain('deployment:')
        ...
        ->and($deployScript)->toContain('Linking public storage uploads into')
        ->and($deployScript)->toContain('ln -sfn "$APP_ROOT/storage/app/public" "$WEB_ROOT/storage"')
        ->and($deployScript)->toContain('Symlink unavailable, syncing public storage uploads into')
