...
test('cpanel deployment files exist and reference the deploy script', function () {
    ...
    expect($deploymentConfig)->toContain('deployment:')
        ...
        ->and($deployScript)->toContain('configure_public_disk_root')
        ->and($deployScript)->toContain('set_env_value "PUBLIC_DISK_ROOT" "$public_storage"')
        ->and($deployScript)->toContain('Publishing public storage uploads at $public_storage')
        ->and($deployScript)->toContain('Replacing $public_storage symlink with a real directory for live uploads')
        ->and($deployScript)->toContain('Merging existing Laravel public disk files into $public_storage')
        ->and($deployScript)->toContain('copy_directory_contents "$app_public_storage" "$public_storage"')
        ->and($deployScript)->not->toContain('ln -s "$app_public_storage" "$public_storage"')
        ->and($deployScript)->not->toContain('rsync -a --delete "$app_public_storage/" "$public_storage/"')
